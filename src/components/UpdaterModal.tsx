...
// Allow ?updater=1 or ?preview_update=<version> to preview the modal in dev/screenshot mode
function getMockParam() {
  try { return new URL(window.location.href).searchParams.get('updater') === '1' } catch { return false }
}

function getPreviewUpdate(): UpdateInfo | null {
  try {
    const params = new URL(window.location.href).searchParams
    const version = params.get('preview_update')
    if (!version) return null
    const bodyParam = params.get('preview_body')
    const body = bodyParam
      ?? `## What's new in v${version}\n\n- New features and improvements`
    return { version, body, date: new Date().toISOString() }
  } catch { return null }
}
...
export function UpdaterModal({ dismissed: dismissedProp, onDismiss, onUpdateAvailable }: UpdaterModalProps = {}) {
  ...
  const [state, setState] = useState<UpdaterState>(
    getMockParam() ? { status: 'available', info: MOCK_UPDATE }
    : previewUpdate ? { status: 'available', info: previewUpdate }
    : { status: 'idle' }
  )
  const [localDismissed, setLocalDismissed] = useState(false)
  // When parent explicitly un-dismisses (re-trigger from bell), reset local state too
  ...
  const checkForUpdate = useCallback(async () => {
    if (getMockParam() || getPreviewUpdate()) return // already pre-populated
    try {
            ...
            >
              ...
              {state.status === 'available' && (
                <p className="text-center text-text-muted/60 text-[10px] px-4 pb-2 leading-relaxed">
                  DataOrbit is in early release — we're actively building it and shipping improvements constantly.
                </p>
              )}
